...
export async function PUT(
  ...
  try {
    ...
    // If status / kanban phase changed, trigger transactional WhatsApp dispatch simulation
    if (prevOrder.status !== updated.status) {
      try {
        await fetch(`${process.env.NEXT_PUBLIC_APP_URL || "http://127.0.0.1:3000"}/api/whatsapp`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            action: "send_message",
            phone: updated.clientPhone || "(11) 98877-6655",
            clientName: updated.clientName,
            templateCode: updated.status,
            message: `Olá ${updated.clientName}! O status do seu pedido *${updated.code}* foi atualizado para: *${updated.status.toUpperCase()}*. Acompanhe em: https://printflow.com.br/rastreio/${updated.code}`,
          }),
        });
      } catch {
        // non-blocking
